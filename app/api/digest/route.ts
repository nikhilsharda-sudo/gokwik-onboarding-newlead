...
export async function GET(req: NextRequest) {
  // Protect with admin token or a cron secret
  ...
  const stale             = all.filter(l => {
    ...
    return now - last > 3 * 24 * 60 * 60 * 1000 && l.status !== "Qualified" && l.status !== "Rejected";
  });
  const byStatus = {
  ...
  const newLeadRows = newThisWeek.map(l => `
    <tr>
      <td style="padding:10px 14px;border-bottom:1px solid #ede9fe;font-weight:700;color:#1a1a2e;font-size:13px">${l.brand_name} ${(l.score||0) >= 70 ? "🔥" : ""}</td>
      ...
      <td style="padding:10px 14px;border-bottom:1px solid #ede9fe;font-size:12px;color:#aaa">${new Date(l.submitted_at).toLocaleDateString("en-IN",{day:"numeric",month:"short"})}</td>
    </tr>`).join("");
  ...
  const staleRows = stale.slice(0, 5).map(l => `
    <tr>
      ...
      <td style="padding:8px 14px;border-bottom:1px solid #ede9fe;font-size:12px;color:#aaa">${Math.floor((now - new Date(l.last_updated||l.submitted_at).getTime()) / 86400000)} days ago</td>
    </tr>`).join("");
...
<table width="620" cellpadding="0" cellspacing="0" style="max-width:620px;width:100%;border-radius:20px;overflow:hidden;box-shadow:0 8px 40px rgba(108,59,255,0.12)">
  ...
  <tr><td style="background:white;padding:24px 32px">
    <table width="100%" cellpadding="0" cellspacing="0">
      <tr>
        ${[
          { label:"Total Leads",   val: all.length,            color:"#6C3BFF" },
          { label:"New This Week", val: newThisWeek.length,    color:"#00C48C" },
          { label:"High Value 🔥", val: highValue.length,      color:"#FFB800" },
          { label:"Need Follow-up",val: stale.length,          color:"#FF6B6B" },
        ].map(s => `
          <td style="text-align:center;padding:0 8px">
            <div style="background:#f8f7ff;border-radius:14px;padding:14px 10px">
              ...
              <div style="font-size:11px;color:#888;margin-top:4px;font-weight:500">${s.label}</div>
            </div>
          </td>`).join("")}
      </tr>
    </table>
  ...
  <tr><td style="background:white;padding:0 32px 24px">
    <h2 style="font-size:14px;font-weight:800;color:#1a1a2e;margin:0 0 12px">🆕 New Leads This Week (${newThisWeek.length})</h2>
    <table width="100%" cellpadding="0" cellspacing="0" style="border-radius:12px;overflow:hidden;border:1px solid #ede9fe">
      <tr style="background:#f8f7ff">
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Brand</th>
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">GMV</th>
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Source</th>
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Status</th>
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Date</th>
      </tr>
      ${newLeadRows}
    </table>
  </td></tr>` : ""}
  ...
  ${stale.length > 0 ? `
  <tr><td style="background:white;padding:0 32px 24px">
    <h2 style="font-size:14px;font-weight:800;color:#FF6B6B;margin:0 0 12px">⚠️ Stale Leads Needing Attention (${stale.length})</h2>
    <table width="100%" cellpadding="0" cellspacing="0" style="border-radius:12px;overflow:hidden;border:1px solid #ede9fe">
      <tr style="background:#FEF2F2">
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Brand</th>
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Status</th>
        <th style="padding:8px 14px;text-align:left;font-size:11px;color:#888;font-weight:700;text-transform:uppercase">Last Touched</th>
      </tr>
      ${staleRows}
    </table>
  </td></tr>` : ""}
  ...
  <tr><td style="background:white;padding:0 32px 28px;text-align:center">
    <a href="${process.env.NEXT_PUBLIC_APP_URL || "https://your-app.vercel.app"}/admin"
  ...
  await resend.emails.send({
    ...
    subject: `📊 Weekly Lead Digest — ${newThisWeek.length} new, ${stale.length} stale · GoKwik`,
    html,
  });

  return NextResponse.json({ success: true, sent: true, newLeads: newThisWeek.length, stale: stale.length });
}